...
#Tegemist on bait adresseeritava mäluga

class Memory():
    def __init__(self, size): #Mälukogus peaks olema baitides (max 65536 baiti)
        assert size <= 32765, f"Max mälukogus on 32765 baiti, sina sisestasid aga: {size} baiti" 
    ...
    def store(self, value, address): #Paigutab väärtuse value mällu vastavale aadressile
        ...
        assert 0 <= address < len(self.memory), f"Aadress {address} on mälupiirkonnast väljas"
